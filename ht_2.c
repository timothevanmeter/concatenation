...
char* ht_set_entry(ht_entry* entries, size_t capacity,
        char* key, int* counts, size_t* plength) {
    // AND hash with capacity-1 to ensure it's within entries array.
    uint64_t hash = hash_key(key);
    ...
    while (entries[index].key != NULL) {
	  ...
	  /* INSTEAD OF UPDATING THE COUNT WE HAD ANOTHER ENTRY */
	  /*   IN THE COUNT ARRAY */	  
	  entries[index].counts = counts;
...
char* ht_set(ht* table, char* key, int* counts) {
    assert(counts != NULL);
    if (counts == NULL) {
        return NULL;
    }
    ...
    return ht_set_entry(table->entries, table->capacity, key, counts,
                        &table->length);
}
